...
public:
    ...
    uint8_t *GetSpace(uint32_t &myInx,int32_t num)
    {
		...
		AtomicXchgAdd(CitWriter,(uint32_t)2); //register writer
        myInx = CitWriter & 0x01;
    ...
    {
        if (myInx != (CitWriter & 0x01)){  
            AtomicInc(Buffs[myInx^1].nWriter);
        }
        else{
            AtomicXchgAdd(CitWriter,(uint32_t)-2);
